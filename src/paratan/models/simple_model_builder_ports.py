...
def parse_simple_machine_input(input_data, material_ns):
    ...
    vv_params = {
        "geometry_style": vv_info["geometry_style"],
    ...
    # Port layers parameters
    ports_info = input_data.get("ports", {})
    ports_params = {
        "ports_present": ports_info.get("ports_present", True),
        "inner_radius": ports_info.get("inner_radius", 25),
        "port_layers_thicknesses": ports_info.get("port_layers_thicknesses", []),
        "port_layers_materials": [getattr(material_ns, "vacuum")] + [getattr(material_ns, material) for material in ports_info.get("port_layers_materials", [])],
        "port_x_limits": ports_info.get("port_x_limits", 450)
    }

    return vv_params, cc_params, lf_params, hf_params, end_params, ports_params
...
class SimpleVacuumVessel:
    ...
    def build_perpendicular_vacuum_vessel(self, room_region):
        """Build the perpendicular vacuum vessel structure using perpendicular_vacuum_vessel_region."""
        self.room_region = room_region
        
        # Create perpendicular vacuum vessel region using the new function
        vv_perpendicular_region, vv_perpendicular_components = perpendicular_vacuum_vessel_region(
            self.vv_params["central_axial_length"], 
            self.vv_params["central_radius"],
            self.vv_params["bottleneck_radius"],
            self.vv_params["left_bottleneck_length"],
        ...
        # Create perpendicular vacuum vessel cell
        vv_perpendicular_cell = openmc.Cell(
            1000,
            region=vv_perpendicular_region,
            fill=self.vv_params["vacuum_material"]
        )
        
        self.vacuum_section_regions.append(vv_perpendicular_region)
        self.vacuum_section_cells.append(vv_perpendicular_cell)

        # Build structural layers as nested composite shapes (following run_simple_mirror_model logic)
        if "structure" in self.vv_params:
            structural_thicknesses = [properties.get("thickness") for layer_name, properties in self.vv_params["structure"].items()]
            structural_thicknesses_array = np.array(structural_thicknesses)
            ...
            central_radii = np.cumsum([self.vv_params["central_radius"]] + structural_thicknesses)
            bottleneck_radii = np.cumsum([self.vv_params["bottleneck_radius"]] + structural_thicknesses)
            central_axial_lengths = self.vv_params["central_axial_length"] + np.cumsum(2*structural_thicknesses_array)
            bottleneck_axial_lengths = np.cumsum(structural_thicknesses)

            # Build structural layers as nested composite shapes
            for i, (thickness, material) in enumerate(zip(structural_thicknesses, structural_materials)):
                # Create the full vacuum vessel region at this structural layer's outer radius
                # Use BOTH updated radii: central_radii[i+1] and bottleneck_radii[i+1]
                vv_layer_region, _ = perpendicular_vacuum_vessel_region(
                    central_axial_lengths[i],
                    central_radii[i + 1],
                    bottleneck_radii[i + 1],
                    self.vv_params["left_bottleneck_length"]-bottleneck_axial_lengths[i],
                    self.vv_params["right_bottleneck_length"]-bottleneck_axial_lengths[i],
                    self.vv_params["axial_midplane"]
                )
...
class SimpleCentralCell:
    ...
    def build_central_cell(self, outer_radius, room_region, vacuum_vessel_regions=None):
        ...
        self.central_cell_regions = [first_cell_region]
        ...
        for i in range(1, len(self.central_cell_cylinders_radii)):
            ...
            # Add regions to the central cell regions list        
            self.central_cell_regions.append(cyl_region)
...
class SimpleHFCoilBuilder:
    ...
    def build_hf_coils(self, central_cell_length_from_midplane, bottleneck_cylinders_radii, room_region, hf_z0_offset=0):
        ...
        vv_outermost_radius = bottleneck_cylinders_radii[0] + self.hf_params["shield"]["radial_gap_before_casing"] # This is the VV outer radius
    ...
    def get_left_hf_coil_cell(self):
        """Get the left HF coil cell."""
        return self.hf_coil_cells[0]
    
    def get_right_hf_coil_cell(self):
        """Get the right HF coil cell."""
        return self.hf_coil_cells[1]
...
class SimpleMirrorPorts:
    """Builds the ports for simple mirror machine."""
    
    def __init__(self, ports_params, material_ns):
        self.ports_params = ports_params
        self.material_ns = material_ns
        self.ports_cells = []
        self.ports_regions = []
        self.tally_descriptors = []
        
    def build_ports(self, ports_params, material_ns, room_region, vacuum_section_regions):
        """Build the ports."""

        port_inner_radius = self.ports_params["inner_radius"]
        port_layers_thicknesses = self.ports_params["port_layers_thicknesses"]
        port_layers_materials = self.ports_params["port_layers_materials"]
        port_x_limits = self.ports_params["port_x_limits"]

        x_min = openmc.XPlane(x0=-port_x_limits)
        x_max = openmc.XPlane(x0=port_x_limits)
        
        innermost_port_cylinder = openmc.ZCylinder(r=port_inner_radius)
        innermost_port_region = -innermost_port_cylinder
        port_layers_regions = [innermost_port_region]

        port_layers_radii_array = port_inner_radius + np.cumsum(port_layers_thicknesses)
        port_layers_cylinders = [openmc.ZCylinder(r=r) for r in port_layers_radii_array]

        for i in range(len(port_layers_thicknesses)):
            port_layer_region = -port_layers_cylinders[i] & +innermost_port_cylinder
            port_layers_regions.append(port_layer_region)
            innermost_port_cylinder = port_layers_cylinders[i]


        # Rotate the finished regions ONCE (No redundant for-loop)
        left_port_regions = [(region.rotate([0, 45, 0]) & +x_min & -x_max) for region in port_layers_regions]
        right_port_regions = [(region.rotate([0, 315, 0]) & +x_min & -x_max) for region in port_layers_regions]

        # Exclude the first region of the vacuum section from the port layers
        for port_region in left_port_regions + right_port_regions:
            port_region &= ~vacuum_section_regions[0]
        
                
        self.ports_regions = left_port_regions + right_port_regions

        left_port_cells = [openmc.Cell(7010 + i, region=region, fill=port_layers_materials[i]) for i, region in enumerate(left_port_regions)]
        right_port_cells = [openmc.Cell(7020 + i, region=region, fill=port_layers_materials[i]) for i, region in enumerate(right_port_regions)]
        
        self.ports_cells = left_port_cells + right_port_cells
        return self.ports_cells
    
    def get_ports_combined_region(self):
        """Get the combined region of the ports."""
        if not self.ports_regions:
            return None
        combined_region = self.ports_regions[0]
        for region in self.ports_regions[1:]:
...
class SimpleMachineBuilder:
    ...
    def __init__(self, vv_params, cc_params, lf_params, hf_params, end_params, ports_params, material_ns):
        self.vv_params = vv_params
        self.cc_params = cc_params
        self.lf_params = lf_params
        self.hf_params = hf_params
        ...
        self.ports = SimpleMirrorPorts(ports_params, material_ns)
    ...
    def build_perpendicular_vacuum_vessel(self):
        """Build the perpendicular vacuum vessel."""
        ...
        vv_cells, vv_regions = self.vacuum_vessel.build_perpendicular_vacuum_vessel(self.room_region)
        for cell in vv_cells:
            self._add_cell(cell)
        self._subtract_from_room(vv_regions)
    ...
    def build_ports(self):
        """Build the ports."""
        ports_cells = self.ports.build_ports(self.ports_params, self.material_ns, self.room_region, self.vacuum_vessel.vacuum_section_regions)
        for cell in ports_cells:
            self._add_cell(cell)
        
        for cell in self.vacuum_vessel.vacuum_section_cells:
            cell.region &= ~self.ports.get_ports_combined_region()
        self._subtract_from_room(self.ports.ports_regions)
    ...
    def build_central_cell(self):
        ...
        # Subtract the central cell regions from the room region
    ...
    def build_lf_coils(self):
        ...
        # Subtract the LF cells from the room region
...
def build_simple_model_from_input(input_data, output_dir="."):
    ...
    with change_dir(output_dir):
        ...
        vv_params, cc_params, lf_params, hf_params, end_params, ports_params = parse_simple_machine_input(input_data, m)

        # Create builder
        builder = SimpleMachineBuilder(
            vv_params, cc_params, lf_params, hf_params, end_params, ports_params, m
        )

        # Build components - add central cell, LF coils, HF coils, and end cells
        if builder.vv_params["geometry_style"] == "conical":
            builder.build_vacuum_vessel()
        elif builder.vv_params["geometry_style"] == "perpendicular":
            builder.build_perpendicular_vacuum_vessel()
        else:
            raise ValueError(f"Unsupported geometry style: {builder.vv_params['geometry_style']}")
        builder.build_ports() 
        ...
        # Mesh based weight window generator for the hf coil meshes

        # Get the left and right HF coil cells
        left_hf_coil_cell = builder.hf_coils.get_left_hf_coil_cell()
        right_hf_coil_cell = builder.hf_coils.get_right_hf_coil_cell()

        # Choose cylindrical mesh resolution for weight windows: (nr, nphi, nz)
        hf_mesh_dims = [20, 1, 20]  # this is hard coded for now but technically needs information from the input parameters

        # Get the mesh for the left and right HF coil cells
        left_hf_coil_mesh = hollow_mesh_from_domain(left_hf_coil_cell, hf_mesh_dims)
        right_hf_coil_mesh = hollow_mesh_from_domain(right_hf_coil_cell, hf_mesh_dims)

        print(f"Left mesh dimensions: {left_hf_coil_mesh.dimension}")
        print(f"Right mesh dimensions: {right_hf_coil_mesh.dimension}")
        print(f"Left mesh: {left_hf_coil_mesh.lower_left}")
        print(f"Right mesh: {right_hf_coil_mesh.lower_left}")

        wwg_left_hf_coil = openmc.WeightWindowGenerator(
            left_hf_coil_mesh,
        ...
        wwg_right_hf_coil = openmc.WeightWindowGenerator(
            right_hf_coil_mesh,
            energy_bounds=[0, 14e6],
            particle_type='neutron',
            method='magic',
        ...
        print(f"Created weight window generators for left and right HF coils")

        settings.weight_window_generators = [wwg_left_hf_coil, wwg_right_hf_coil]
